...
public class PrintListaByMarcaPage extends AbstractPrint implements Printable {
	...
	public PrintListaByMarcaPage(List<Articulo> articulos, int indexFrom, int indexTo, int pageNum) {		
	...
	public int print(Graphics g, PageFormat pf, int pageIndex) {
		...
		for (int i = this.indexFrom; i < this.indexTo && !finPage; ) {
			...
			//PRINT Marca
			if(printMarca(articuloAnt, i, this.articulos)){				
				...
				g2d.drawString(articuloAnt.getMarca().getId().toString() + "  " + articuloAnt.getMarca().getDescripcion().toUpperCase(), xPos, yPos);
			}
			
			//PRINT Familia
			if(printFamilia(articuloAnt, i, this.articulos) && printMarca(articuloAnt, i, this.articulos)){
				yPos += PULGADA * 1.5;
				...
				g2d.setFont(fontS10);
				g2d.drawString(articuloAnt.getFamilia().getCodigo().toString() + "  " + articuloAnt.getFamilia().getDescripcion().toUpperCase(), xPos  + 10, yPos);
			}else if(printFamilia(articuloAnt, i, this.articulos)){
				if(isFinPage(xPos, yPos, 17)){						
					break;
				}
				//coordinates
			...
			//PRINT Articulo			
			if(yPos != 45 || (!printMarca(articuloAnt, i, this.articulos) && printFamilia(articuloAnt, i, this.articulos)))
				yPos += PULGADA * 1.5;
			g2d.setFont(fontS9);
